...
// G��G��G�� Date Utilities (business days: Mon-Fri) G��G��G��G��G��G��G��G��G��G��
...
// G��G��G�� Core Engine G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
...
function applyCompletionRules(task: ScheduleTask): ScheduleTask {
  ...
  // If actual_finish exists G�� complete
  ...
  // If actual_start exists but no actual_finish G�� in_progress
...
function buildDependencyMaps(tasks: ScheduleTask[]): {
  ...
  const idLookup = new Map<string, string>(); // activity_id G�� uuid
  ...
  for (const task of tasks) {
    ...
    if (task.total_float !== null && task.total_float < 0) {
      flags.push({
        ...
        description: `${task.activity_name} has ${task.total_float} days negative float G�� schedule is being pushed`,
        severity: "high",
      });
    }
    ...
    if (task.total_float === 0 && !task.milestone) {
      flags.push({
        ...
        description: `${task.activity_name} has zero float G�� any delay will push the project`,
        severity: "medium",
      });
    }
    ...
    if (task.milestone && task.total_float !== null && task.total_float <= 3) {
      flags.push({
        ...
        description: `Milestone "${task.activity_name}" has ${task.total_float} days float G�� at risk of delay`,
        severity: task.total_float <= 0 ? "high" : "medium",
      });
    }
  }
...
function generateRecoveryActions(
  ...
  for (const task of criticalWithPreds.slice(0, 2)) {
    actions.push({
      priority: priority++,
      category: "overlap",
      description: `Evaluate overlapping "${task.activity_name}" with its predecessor G�� start when predecessor is 80% complete instead of waiting for full completion`,
      target_task_id: task.id,
      target_task_name: task.activity_name,
  ...
  for (const task of delayedWithLongDuration.slice(0, 2)) {
    actions.push({
      ...
      description: `Verify material/equipment procurement status for "${task.activity_name}" G�� confirm long-lead items are ordered and delivery dates won't cause additional delay`,
      target_task_id: task.id,
      target_task_name: task.activity_name,
...
// G��G��G�� Main Entry Point G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��G��
