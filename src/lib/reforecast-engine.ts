...
import { getArizonaToday } from "@/lib/arizona-date";`r`n`r`n// GöÇGöÇGöÇ Types GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
...
// GöÇGöÇGöÇ Date Utilities (business days: Mon-Fri) GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
...
/** Get today's date as YYYY-MM-DD in Arizona timezone */
function today(): string {
  // Import is at top of file
  return getArizonaToday();
}

// GöÇGöÇGöÇ Core Engine GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
...
function applyCompletionRules(task: ScheduleTask): ScheduleTask {
  ...
  // If actual_finish exists GåÆ complete
  ...
  // If actual_start exists but no actual_finish GåÆ in_progress
...
function buildDependencyMaps(tasks: ScheduleTask[]): {
  ...
  const idLookup = new Map<string, string>(); // activity_id GåÆ uuid
  ...
  for (const task of tasks) {
    ...
    if (task.total_float !== null && task.total_float < 0) {
      flags.push({
        ...
        description: `${task.activity_name} has ${task.total_float} days negative float GÇö schedule is being pushed`,
        severity: "high",
      });
    }
    ...
    if (task.total_float === 0 && !task.milestone) {
      flags.push({
        ...
        description: `${task.activity_name} has zero float GÇö any delay will push the project`,
        severity: "medium",
      });
    }
    ...
    if (task.milestone && task.total_float !== null && task.total_float <= 3) {
      flags.push({
        ...
        description: `Milestone "${task.activity_name}" has ${task.total_float} days float GÇö at risk of delay`,
        severity: task.total_float <= 0 ? "high" : "medium",
      });
    }
  }
...
function generateRecoveryActions(
  ...
  for (const task of criticalWithPreds.slice(0, 2)) {
    actions.push({
      priority: priority++,
      category: "overlap",
      description: `Evaluate overlapping "${task.activity_name}" with its predecessor GÇö start when predecessor is 80% complete instead of waiting for full completion`,
      target_task_id: task.id,
      target_task_name: task.activity_name,
  ...
  for (const task of delayedWithLongDuration.slice(0, 2)) {
    actions.push({
      ...
      description: `Verify material/equipment procurement status for "${task.activity_name}" GÇö confirm long-lead items are ordered and delivery dates won't cause additional delay`,
      target_task_id: task.id,
      target_task_name: task.activity_name,
...
// GöÇGöÇGöÇ Main Entry Point GöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇGöÇ
