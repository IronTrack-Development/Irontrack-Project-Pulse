...
export default function FieldReportDetail({ projectId, report: initialReport, onBack, getPhotoUrl }: Props) {
  ...
  const handleDelete = async () => {
    if (!confirm(t('reports.deleteThisReport'))) return;
    setDeleting(true);
  ...
  const handleDeletePhoto = async () => {
    if (!confirm(t('reports.removePhotoConfirm'))) return;
  ...
  const handleMarkup = () => {
    ...
    if (typeof window !== "undefined") {
      ...
      el.textContent = t('reports.markupComingSoon');
      el.className =
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      {/* Title */}
      <div className="mb-4">
        <label className="block text-xs text-[color:var(--text-muted)] mb-1">{t('reports.titleLabel')}</label>
        <input
          type="text"
          value={report.title}
      ...
      {/* Assigned To */}
      <div className="mb-4">
        <label className="block text-xs text-[color:var(--text-muted)] mb-1">{t('reports.assignedTo')}</label>
        <input
          type="text"
      ...
      {/* Comments */}
      <div className="mb-4">
        <label className="block text-xs text-[color:var(--text-muted)] mb-1">{t('reports.comments')}</label>
        <textarea
      ...
      {showMore && (
        <div className="space-y-4 mb-6 pl-1">
          {/* Location */}
          <div>
            <label className="block text-xs text-[color:var(--text-muted)] mb-1">{t('reports.location')}</label>
            <input
              type="text"
          ...
          {/* Priority */}
          <div>
            <label className="block text-xs text-[color:var(--text-muted)] mb-1">{t('reports.priority')}</label>
            <select
          ...
          {/* Status */}
          <div>
            <label className="block text-xs text-[color:var(--text-muted)] mb-1">{t('reports.status')}</label>
            <select
              value={report.status}
              onChange={(e) => {
          ...
          {/* Trade */}
          <div>
            <label className="block text-xs text-[color:var(--text-muted)] mb-1">{t('reports.trade')}</label>
            <input
              type="text"
              value={report.trade || ""}
          ...
          {report.status === "resolved" && (
            <div>
              <label className="block text-xs text-[color:var(--text-muted)] mb-1">{t('reports.resolutionNotes')}</label>
              <textarea
      ...
      {/* Saving indicator */}
      {saving && (
        <p className="text-xs text-[color:var(--text-muted)] mb-3">{t('reports.saving')}</p>
      )}
      ...
      >
        <Trash2 size={14} />
        {deleting ? t('reports.deleting') : t('reports.deleteReport')}
      </button>
    </div>
  );
}
