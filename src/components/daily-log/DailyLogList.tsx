      ...
      <div className="flex items-start justify-between mb-2">
        <div>
          ...
          <div className="text-xs text-[color:var(--text-muted)] mt-0.5">
            {log.superintendent || t('dailylog.superintendent')}
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-3 text-xs text-[color:var(--text-secondary)] mt-2">
        ...
        {totalHeadcount > 0 && (
          <span className="flex items-center gap-1">
            ...
            {totalHeadcount} {t('dailylog.crew')}
          </span>
        )}
        {(log.delay_codes || []).length > 0 && (
          <span className="flex items-center gap-1">
            <AlertTriangle size={12} className="text-[#EAB308]" />
            {log.delay_codes.length} {t('dailylog.delays')}
          </span>
        )}
      </div>
    </Link>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Plus size={14} />
            {hasTodayLog ? t('dailylog.editToday') : t('dailylog.newLog')}
          </Link>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-[color:var(--text-muted)] mb-1">{t('dailylog.noLogs')}</p>
          <p className="text-xs text-gray-600">{t('dailylog.startFirst')}</p>
        </div>
      ) : (
