            ...
            {formItems.map((item, idx) => (
              <div key={idx} className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-[var(--bg-tertiary)] border border-[var(--border-secondary)] rounded-lg text-sm text-[color:var(--text-primary)] placeholder-gray-500 min-h-[36px]"
                />
                {formItems.length > 1 && (
        ...
        Object.entries(grouped).map(([key, temps]) => (
          <div key={key}>
            <div className="flex items-center gap-2 mb-2">
              <ArrowRightLeft size={12} className="text-[color:var(--text-muted)]" />
