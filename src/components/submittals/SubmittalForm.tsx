    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
      <div className="bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[92dvh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-[var(--bg-secondary)] border-b border-[var(--border-primary)] px-5 py-4 flex items-center justify-between z-10">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-lg text-gray-400 hover:text-white hover:bg-[var(--bg-tertiary)] transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-gray-400 mb-1.5 font-medium">Number *</label>
              <input
                value={form.submittal_number}
                onChange={(e) => setForm((f) => ({ ...f, submittal_number: e.target.value }))}
                placeholder="S-001"
                required
                className="w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-[#F97316]/50 placeholder-gray-600"
              />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5 font-medium">Spec Section</label>
              <input
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-[#F97316]/50 placeholder-gray-600 resize-none"
            />
          </div>
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors min-h-[36px] ${
                    form.priority === p.value
                      ...
                      : "bg-[var(--bg-primary)] border border-[var(--border-primary)] text-gray-400 hover:text-white"
                  }`}
                >
                  {p.label}
                </button>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-gray-400 mb-1.5 font-medium">Required By</label>
              <input
                type="date"
                value={form.required_by}
                onChange={(e) => setForm((f) => ({ ...f, required_by: e.target.value }))}
                className="w-full bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-[#F97316]/50"
              />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5 font-medium">Lead Time (days)</label>
              <input
                type="number"
                min={0}
          ...
          {/* Assigned To */}
          <div>
            <label className="block text-xs text-gray-400 mb-1.5 font-medium">Assigned To (Sub/Supplier)</label>
            <select
          ...
          {/* Reviewer */}
          <div>
            <label className="block text-xs text-gray-400 mb-1.5 font-medium">Reviewer (Architect/Engineer)</label>
            <select
          ...
          {/* Ball in Court */}
          <div>
            <label className="block text-xs text-gray-400 mb-1.5 font-medium">Ball in Court</label>
            <div className="flex gap-2 flex-wrap">
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-3 rounded-xl bg-[var(--bg-tertiary)] text-gray-300 hover:text-white text-sm font-medium transition-colors min-h-[44px]"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={saving}
              className="flex-1 py-3 rounded-xl bg-[#F97316] hover:bg-[#ea6c10] disabled:opacity-50 text-white text-sm font-bold transition-colors min-h-[44px] flex items-center justify-center gap-2"
            >
