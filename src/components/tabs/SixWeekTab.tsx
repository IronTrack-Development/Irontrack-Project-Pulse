...
function statusChip(status: string, pct: number) {
  ...
  return (
    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-[var(--bg-tertiary)] text-gray-500 border border-[#2a2a33]">
      Upcoming
    </span>
  );
}
    ...
    <div className="flex items-start justify-between gap-3 py-3 px-4 bg-[#13131A] border border-[var(--border-primary)] rounded-xl">
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-1 flex-wrap">
          ...
          {activity.trade && (
            <span className="text-[10px] bg-[var(--bg-tertiary)] text-gray-400 px-1.5 py-0.5 rounded">
              {activity.trade}
            </span>
          )}
          {days !== null && days > 0 && (
