# Sub Ops V1 — Complete Build Spec

## Overview
A subcontractor field operations platform built into IronTrack Pulse. Enables sub company owners/PMs to manage foremen, dispatch daily assignments, track production, and store SOPs. Accessible via the sub portal or as a standalone sub login.

## Database — Migration 021_sub_ops.sql
...
### Table: `sub_dispatches`
The morning huddle — daily assignments from owner/PM to foremen.
```sql
...
### Table: `sub_checkins`
Foreman daily check-in from the field.
```sql
...
### Table: `sub_production_logs`
What got installed/completed today.
```sql
...
### Table: `sub_blockers`
Issues/blockers reported by foremen.
```sql
...
### Table: `sub_sop_acknowledgments`
Track who has read/acknowledged each SOP.
```sql
...
### Table: `sub_dispatch_sops`
SOPs attached to a specific dispatch.
```sql
